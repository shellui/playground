...
- Avoid a permanent blank companion iframe when `shellui.init()` never resolves. SDK 0.5.2/0.5.3 still wait forever for `SHELLUI_SETTINGS` in `init()` (the `waitForInitialSettings` helper ships in 0.5.3 but is not wired). Bootstrap now times out the handshake (~2.5s) and still paints; hard failures show an on-page error instead of a white screen.

### 🛠 Improvements

- Bump to Shellui [0.5.3](https://github.com/shellui/shellui/releases/tag/v0.5.3) (`@shellui/cli`, `@shellui/core`, `@shellui/sdk`).
